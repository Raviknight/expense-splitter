...
| 14 | 🔴 **Performance — still unexplained; needs a browser profile** | **Ruled out by measurement, not assumption:** network latency (Supabase round trip 120–350ms), asset size (bundle transfers at **155KB gzipped in 673ms**, styles.css 6.9KB — GitHub Pages does compress), CSS compilation (Play CDN removed). Five sequential queries were fixed too. Bundle composition measured: react-dom 127KB, App.jsx 104KB, supabase auth-js 59KB. **Assets and data together should be ~1–2s, so something else is responsible.** Remaining suspects, none confirmed: realtime websocket setup or retry churn; the service worker's network-first navigation blocking on a slow response; Supabase free-tier cold start (first request measured 1060ms vs 120ms warm). NEXT STEP IS A REAL PROFILE from the owner's browser (DevTools → Network, hard reload, note the slowest request and total load) — a fourth round of guessing is not worth anyone's time. |
| 14b | ~~Performance — measure before guessing~~ (folded into 14) | Reported: ~30s to load the dashboard, Connections slow too. Ruled out: network (Supabase round trip measured 120–350ms). Fixed: five sequential queries → three parallel stages. Still unexplained if it persists — next step is a real profile (DevTools Performance + Network) rather than more speculation. Suspects: Play CDN (#13), 558KB bundle, realtime refetching on every change, unbounded `select('*')` on expenses. |
