...
| 8 | **Deploy drift on Edge Functions** — CI deploy built, blocked on one GitHub secret | Functions were deployed by pasting into the dashboard, so the repo and the running code were two copies with nothing keeping them in step. That drift already cost real debugging time once: a stale deployed copy produced the misleading "Gemini not active" error, which made no sense against the code in the repo. `.github/workflows/deploy-functions.yml` now deploys every function on any push to `main` touching `supabase/functions/`, so main IS the deployment. **Blocked only on the owner adding a `SUPABASE_ACCESS_TOKEN` repo secret** (supabase.com/dashboard/account/tokens → Generate new token → GitHub repo Settings → Secrets → Actions). `verify_jwt` is declared per function in `supabase/config.toml` and was **measured, not assumed** — each function was called with no Authorization header and all four returned the gateway's own `UNAUTHORIZED_NO_AUTH_HEADER`, so verification is on everywhere and the CLI default matches production. That check mattered most for `send-welcome`, which a DATABASE WEBHOOK calls rather than the app — the one that could plausibly have needed it off, where a wrong value would have stopped welcome emails silently. **First run overwrites the dashboard with the repo's copy**, so confirm the repo is the good version before enabling; after that the question stops existing. **SQL migrations are deliberately OUT of scope** and `config.toml` says so loudly where someone reaching for a `db` command will meet it: all 25 `db/` files were applied by hand, the CLI's migration ledger is empty (which is what the dashboard's harmless "schema_migrations does not exist" log noise actually is), `supabase db reset` would destroy real expense data, and `db pull` would write one opaque baseline superseding the numbered files that explain WHY each change was made. Baselining those is a separate decision with real risk. |

## Needs verification (built, not proven)

| # | Item | How to prove it |
|---|------|-----------------|
| V1 | iPhone PWA recovers on its own when reopened from the home icon — **STILL NOT FIXED after two attempts; third fix shipped 2026-09-15, unverified** | **Reported again 2026-09-15 with a screenshot: the amber banner, on a phone with signal, with the owner's words "the app does not refresh from server."** Second root cause found, and it is not the same as the first: `retryAttemptRef` was only ever reset on SUCCESS, so once the four fast retries were spent (~30 seconds) the retry chain was finished for the life of the page. Anyone who opened the app during a bad half-minute was left on the banner indefinitely — nothing tried again while they sat there, and only Refresh or a background-and-reopen could rescue it. A recovery that gives up is not a recovery. Now: the fast burst stays (2/4/8/16s, for a waking radio) and then settles into a 60s heartbeat that keeps going while online and stale, and every return to the app resets the budget so reopening tries hard rather than inheriting an exhausted counter. **NOT YET PROVEN.** The give-up was a genuine defect regardless, but it has not been confirmed as the cause of what the owner saw — that needs a phone. **The one question that settles it: when the banner is showing, does tapping Refresh work?** If yes, the server is reachable and this fix is the answer. If Refresh ALSO fails, the cause is something else entirely and this was a fix to a different bug. Earlier history follows. **The earlier "fix" did not fix this — it changed the symptom.** The 2.5s service-worker timeout stopped the infinite spinner, so the app then rendered stale data forever instead. Reported again 2026-09-13 with a screenshot. Root cause found and fixed in `92561bd`: `stale` is only cleared by a COMPLETED `fetchAll` and nothing retried after a failure, so a failed first load was terminal — and on iOS the reopen IS the page load, so the existing resume listener had already fired before that first fetch failed. Now: bounded 2/4/8/16s backoff from both the error path and the 12s watchdog (a hung query never reaches the catch), and the resume refetch no longer waits on `getSession()`, which can hang forever because auth-js ships no fetch timeout. **Test: background it overnight, reopen, and do NOT touch Refresh — the banner must clear by itself.** Still unconfirmed on real hardware. |
