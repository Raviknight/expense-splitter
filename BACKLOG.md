...
| V4 | **UPI hand-off — built, and NOT verifiable from outside India** | The button builds a correct `upi://pay?…` URI (12 parser cases tested, encoding checked), but whether a mobile browser actually hands off to a UPI app is untested. The owner's attempts opened a WhatsApp **chat**, which looked like a bug and is not one: **he is in the US**, where WhatsApp Pay has no UPI capability and BHIM needs an Indian bank and connectivity. A UPI link on a phone with no working UPI app does nothing useful, correctly. **So this is UNVERIFIED, not broken** — do not "fix" it on the strength of that report. **How to prove it:** someone in India, with GPay or PhonePe, in an INR group, taps the button and sees their app open with the amount prefilled. Thirty seconds for them, and impossible for anyone else. If it turns out deep links are unreliable in practice, a **UPI QR code** sidesteps handlers entirely and fixes the desktop case properly at the same time — see the Parking lot. |
