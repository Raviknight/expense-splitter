...
| 23 | 🟡 **In a 3+ person group, members who aren't connected see each other as "Unknown"** — FIX APPLIED to the live database 2026-09-14, still needs a real 3-person group to confirm | **Migrations are in.** Verified against production by probe: `payment_notes` now exists (returns `[]` rather than `PGRST205`), `profiles.payment_note` is gone (`42703`), `profiles` still reads fine, and `shares_group_with()` answers `false` for a signed-out caller rather than 404ing — so the helper both policies depend on is live. What is NOT yet proven is the thing that matters: that a third member actually SEES the other two by name. That needs three real accounts in one group and cannot be probed with an anon key, because the policy turns on `auth.uid()`. | Confirmed wanted by the owner: *"members should be able to see other members name as they wouldn't know who owes what and who they owe money."* Fixed by `db/22_read_co_member_profiles.sql`, which adds a `read co-member profiles` policy on `profiles` backed by a new `shares_group_with()` SECURITY DEFINER helper — `security definer` is load-bearing, since a policy on `profiles` that reads `group_members` would otherwise recurse through `group_members`'s own policy (db/01 solves the identical problem the same way for `is_member_of`). Deliberately NOT fixed by auto-creating connection rows on join: a connection is a mutual relationship the user manages, forging it would put people in each other's contact lists without consent, and — because db/01's policy ignores `status` — a forged row would hand over more than a name. This also forced a correction to `db/21`: its first draft granted payment-note reads to accepted connections ONLY, which would have left the two non-owner members of a group unable to see how to pay each other. Both files now allow "yourself OR accepted connection OR shares a group", still far tighter than today's "any connections row, including declined". Original diagnosis follows. Found while designing #21's accepted-only policy, by asking whether group membership actually implies a connection. It does not. Members are linked only from the GROUP OWNER's accepted connections (`App.jsx:3226` offers `useConnections().accepted` as the sole link candidates), and `db/09`'s invite flow inserts a connection between the **inviter and the new user** only. Nothing ever connects co-members to each other. So in a group of Ravi (owner) + Shailja + Alex, Shailja and Alex have NO connection, the `read connected profiles` policy denies Shailja any read of Alex's row, and `memberDisplayName` (`store.js:63-68`) falls back to the literal string **`'Unknown'`**. Shailja sees "Unknown" in the member list, in balances, and in settle-up suggestions — "Unknown pays Ravi $40". Not yet observed in the wild **only because every existing group is 2-person**, where the single other member is always the owner's connection; it should appear the first time anyone builds a real 3-person group, which is the app's main use case. Worth confirming with a live 3-person group before designing the fix, since the exact shape depends on whether we want co-members auto-connected (changes the meaning of "connection") or a group-scoped read policy on `profiles` (probably the right answer: you can read the profile of anyone you share a group with). Note this is a DISPLAY and privacy-model bug, not a money bug — the balance maths keys off the same names consistently, so totals stay correct even while the labels are wrong. |
