...
| # | Item | Notes |
|---|------|-------|
| 9 | **No way for a user to pay for premium** | Set by hand in Supabase today (`profiles.is_premium`, db/11); `PREMIUM_ENFORCED` is still `false`. Needs a payment-provider decision (a business/tax question — merchant-of-record handles cross-border sales tax, a direct gateway does not). Ask any provider whether they issue **India-format FIRA** before integrating. <br><br>⚠️ **This entry used to say "blocked on flipping `SCAN_LIMIT_ENABLED`", and that was backwards** — corrected 2026-09-15, see Decisions pending. Enforcement without a checkout is a wall with no door: a user who hits the limit cannot upgrade because there is nothing to buy, so they leave. **Payments first, enforcement in the same week.** <br><br>Also waiting on the owner: provider emails to Dodo and Paddle are drafted (`docs-internal/dodo-payments-questions.md`) and deliberately unsent until there are users worth billing. And the pricing shape matters more than the provider — a fixed per-transaction fee makes INR 50/month impossible (you keep ~26%), where the same fee on an annual or one-time price keeps ~89%. A commenter in r/SideProject paid happily for Settle Up's *lifetime, single group* option; that shape is worth considering over a subscription. |
...
| V4 | ✅ **UPI hand-off — VERIFIED 2026-09-19 on an iPhone: BHIM opened with the amount filled in.** Three of five schemes still untested | The mechanism is proven on the platform that looked fatal. iOS gives a custom URL scheme to whichever app claimed it, with no chooser and no way to reassign — so the generic `upi://` opened a WhatsApp *chat* and no device setting could change that. An **app-specific** scheme has exactly one claimant, so there is nothing to disambiguate, and it works. **Still untested: `tez://upi/pay` (GPay), `phonepe://pay`, `paytmmp://pay`** — the owner has neither app. A wrong scheme is a button that silently does nothing, so anyone who has those apps should tap them once and report. `bhim://pay` was the least certain of the five and proved correct, which is encouraging but not evidence about the others. |
...
- ~~**UPI QR code at settle-up**~~ — **DECLINED by the owner 2026-09-19, and not on technical
  grounds.** *"I don't think people will trust a QR code in a splitting app."* Correct, and a
  better reason than the technical case for it: in India a QR reads as a **merchant** payment
  and years of QR-fraud warnings sit behind it, so asking someone to scan in order to repay a
  friend is the wrong register entirely. The technical problem it was meant to solve — iOS
  giving `upi://` to an arbitrary app — was solved instead by per-app schemes (V4), which
  keeps the interaction a tap rather than a scan. Recorded because the QR keeps looking
  attractive on paper: it sidesteps handlers, works on desktop, and is verifiable anywhere.
  None of that outweighs it feeling like a scam risk to the person paying. Original note kept
  below for the reasoning.
- **UPI QR code at settle-up**, rendering the same `upi://pay?…` string the button builds.
  Raised 2026-09-19 after two days of deep-link trouble turned out to be untestable from the
  US. Attractive because it removes the entire class of problem: no scheme handlers, no
  Android defaults, no app chooser, no "which UPI app grabbed it". It also **fixes desktop
  properly** — today a desktop user gets "Copy UPI ID", where a QR is a real payment flow
  (show it, payer scans with their phone), and it is how payment requests are normally made
  in India. It can even be verified from outside India, since a QR either encodes the right
  string or it does not. **Cost:** a QR generator — one dependency against the current five,
  or roughly 200 lines. Decide only after someone in India has tested the deep link (V4): if
  that works, this is a nice-to-have; if it does not, this is the replacement.
