...
| 24 | 🔴 **Corporate mail scanners burn the sign-in token before the user sees it** — CONFIRMED by the project's own auth logs; awaiting the template change | **PROVEN, not inferred.** Supabase auth log: `/otp` 200 at **08:43:00**, then `/verify` 303 **and a completed Login at 08:44:04** — while the email did not reach the Outlook inbox until **08:45**. The token was spent a full minute before the human could read it, so it cannot have been them. An earlier pair shows the same shape (`/otp` 07:59:56 → `/verify` 08:00:31, 35s later). The competing theory — that the mail simply arrives after the 15-minute OTP expiry — is RULED OUT: delivery was only ~1–2 minutes. **Do not "fix" this by raising the OTP expiry; it would change nothing.** Remaining action is the owner's: edit the Supabase **Magic Link** template to send `{{ .Token }}` with NO `{{ .ConfirmationURL }}`, so there is no URL for a scanner to open. The app already supports code entry for sign-in and now for sign-up too. Original diagnosis follows. Reported 2026-09-14: on a work (Outlook/Microsoft 365) address, the magic link AND the emailed code BOTH come back expired. That pairing is the diagnosis. In Supabase the link and the code are **the same single-use token** — `{{ .ConfirmationURL }}` and `{{ .Token }}` are two renderings of one OTP — so anything that consumes the link also kills the code. Microsoft Defender for Office 365 **Safe Links** pre-fetches and detonates every URL in an inbound message to scan it, which spends the token seconds after delivery and long before the human clicks. Proofpoint and Mimecast do the same. This is not flaky mail: it is deterministic, and it will happen on every corporate tenant. **Fix: remove the link from the magic-link template and send the CODE ONLY** — a scanner cannot burn what isn't there. Costs nothing here because the app ALREADY has working code entry (`AuthScreen.jsx` `verifyOtp`, confirmed working by the owner), and code-only is *better* on iPhone anyway, where a link cannot open the home-screen app and leaves it signed out. Template change in the Supabase dashboard, so it needs the owner. **Confirm before changing anything:** the Resend log should show the link CLICKED within seconds of delivery, before the owner touched it — that is the smoking gun, and it distinguishes this from ordinary spam filtering. Separately, the signup confirmation mail never arrived at all, which is a different failure (quarantine, not token burn) — Resend's delivery status for that message will say whether it was delivered, bounced or blocked. |
| 25 | 🔴 **Anyone can delete anyone's expense and leave no trace** | Raised by the owner 2026-09-14: *"If someone accidently delete the expense there is no way to know. especially a person can delete the expense paid by someone else."* Confirmed in code: `store.js:1247` issues a hard `delete()` on the row. There is no `deleted_at`, no audit row, and `countNewActivity` (`App.jsx:428`) only counts expenses that still EXIST — so a deleted expense vanishes from the history that is supposed to explain the balances. Everyone's numbers change and nothing says why or who. For an app whose entire job is telling people what they owe, silent unattributed mutation of the ledger is a trust problem, not a missing nicety. **Recommended shape, two parts.** (a) An audit row rather than a soft delete: a small `expense_deletions` table (group_id, expense_id, description, amount, deleted_by, deleted_at) written on delete, surfaced in the Activity tab as "Ravi deleted 'Hotel' $100". Preferred over `deleted_at` because soft-delete means auditing EVERY existing read for a filter, and one missed filter silently resurrects a deleted expense into someone's balance. (b) An RLS policy so only the PAYER or the group OWNER may delete — visibility tells you it happened, permission stops most of it happening. Needs a migration the owner runs, so it is not unattended work. |
| 26 | 🟠 **The monthly digest fires for everyone at once and will blow the daily email cap — taking sign-in with it** | Raised by the owner, who was right and corrected my arithmetic. I had reasoned that monthly-by-default means 1 email per user per month, so 3,000/month supports ~3,000 users. **That is wrong**: the digest fires for ALL users on the SAME DAY, so it is one burst against Resend's **100/day** free cap, not a flow spread over a month. Real ceiling is ~100 users — identical to daily-digest — and the failure is worse than a missed digest: the burst exhausts the day's quota, so **magic-link codes and sign-up confirmations stop being delivered**, locking people out of the app entirely. The digest is a nice-to-have; the sign-in code is the front door, and today they compete for the same 100. **Fixes, cheapest first:** (a) STAGGER the digest — send each user theirs on the day-of-month they joined, turning one 100-user spike into ~3/day; costs nothing and fixes the shape of the problem rather than raising the ceiling. (b) Reserve headroom: have the digest function stop itself at a daily budget well under the cap, so auth mail always has room. (c) Provider fallback for OUR sends (invite / digest Edge Functions) — try Resend, fall back to a second provider on failure. **(c) does NOT cover auth email**: Supabase takes a single SMTP config, so magic-link and confirmation mail cannot fail over in the app. That asymmetry is the reason (a) and (b) matter more than (c). |
...
| 28 | 🟡 **The digest's dry run reports `"sent"`, which is untrue in a dry run** | Spotted 2026-09-14 when the owner ran the workflow, saw `"sent":1`, and reasonably went looking for an email that was never sent. The response is `{"dryRun":true,"considered":1,"sent":1,"skipped":0,...}` — and `sent` there counts messages that WOULD have gone out. The honest field is already present per recipient (`wouldSend`), so the top-level counter is the only misleading part. Rename it to `wouldSend` when `dryRun` is true, or always report `considered` / `wouldSend` / `skipped` and reserve `sent` for real runs. Small, but the whole purpose of a dry run is to tell you the truth about what will happen, and a field that says "sent" when nothing was sent defeats it. One line in `send-digest`, though it costs a manual redeploy — worth folding into the next change to that function rather than doing alone. |
