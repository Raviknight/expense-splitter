...
| 16 | 🔴 **Invite email never sent** | Invite to a work address never arrived and **never appeared in the Resend log**, meaning `send-invite` never reached Resend — it either isn't deployed, lacks `RESEND_API_KEY`, or errored earlier. Check the function's logs in Supabase first; the answer is almost certainly there. |
